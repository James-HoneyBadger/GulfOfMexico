...
@@- Subset of language features (no satirical keywords, limited functions)
@@- **Does NOT support** Language Construction Set customization
@@- **Use Python interpreter** for language customization and full features
...
@@See `compiler/COMPILER_COMPLETE.md` and `compiler/EXPERIMENTAL_STATUS.md` for details.
...
# By default the interpreter buffers internal debug messages (the `[DB_PRINT]` lines)
# and only prints them if your program errors out — this keeps normal runs clean.
#
# Method 1: Use command-line flags (recommended)
python -m gulfofmexico --debug script.gom          # Show internal debug messages
python -m gulfofmexico --verbose script.gom        # Show completion messages
python -m gulfofmexico.ide --debug                 # Launch IDE with debug output
...
**Customize everything!** Create your own language variants by renaming commands, modifying functions, and adjusting syntax options.
...
python -m gulfofmexico --config my_language.yaml script.gom
```

### Create Your Own Language Variant

```python
from gulfofmexico.language_config import LanguageConfig

# Start from a preset or default
...
config.disable_satirical_keywords()  # Serious mode
...
- **python_like** - Python-style syntax (def, 0-based arrays, # comments)
- **js_like** - JavaScript-style syntax (semicolons, 0-based arrays)
- **serious** - Professional mode (no satirical features)
- **minimal** - Teaching mode (only essential features)
- **spanish** - Spanish keywords and functions
- **french** - French keywords and functions

### What You Can Customize

- **Keywords**: Rename or remove any keyword (if, when, function, class, etc.)
- **Functions**: Rename, disable, or add built-in functions
- **Operators**: Change precedence, add custom operators
- **Array Indexing**: Switch between -1 (Gulf of Mexico) and 0 (traditional)
- **Comments**: Change comment syntax (#, //, --, etc.)
- **Features**: Enable/disable satirical keywords, quantum features, time travel, etc.
- **Language Characteristics**: Toggle 3-valued logic, probabilistic variables, temporal lifetimes

### Documentation

- **[LANGUAGE_CONSTRUCTION_SET.md](LANGUAGE_CONSTRUCTION_SET.md)** - Complete guide with examples
- **[configs/](configs/)** - Example configurations and presets
